...
write_ply( const char* filename, const TriMesh* mesh )
{
  ...
  cube_file.add_properties_to_element("vertex", { "x", "y", "z" }, 
      Type::FLOAT32, 3*mesh->n_verts, reinterpret_cast<uint8_t*>(mesh->vertices), Type::INVALID, 0);

  cube_file.add_properties_to_element("face", { "vertex_indices" },
        Type::UINT32, 3*mesh->n_faces, reinterpret_cast<uint8_t*>((int*)&mesh->faces[0].i1), Type::UINT8, 3);
